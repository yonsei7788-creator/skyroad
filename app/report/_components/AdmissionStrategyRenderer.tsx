...
export const AdmissionStrategyRenderer = ({
  ...
  return (
    <>
      ...
      {/* Block 3: Chance rationale + type strategies */}
      {(data.recommendations.some((r) => r.chanceRationale) ||
        ...
        <div>
          ...
          {data.typeStrategies && data.typeStrategies.length > 0 && (
            <>
              <div
                className={`${styles.h3} ${data.recommendations.some((r) => r.chanceRationale) ? styles.mt24 : ""} ${styles.mb12}`}
              >
                전형별 전략
              </div>
              <table className={styles.compactTable}>
      ...
      {/* Block 3: School type + CSAT + simulation */}
      {(data.schoolTypeAnalysis ||
        ...
        <div>
          ...
          {data.csatMinimumStrategy && (
            <>
              <div
                className={`${styles.h3} ${data.schoolTypeAnalysis ? styles.mt24 : ""} ${styles.mb8}`}
              >
                수능 최저 전략
              </div>
          ...
          {data.applicationSimulation && (
            <>
              <div className={`${styles.h3} ${styles.mt24} ${styles.mb8}`}>
                지원 조합 시뮬레이션
              </div>
      ...
      {/* Block 4: University guide matching */}
      ...
      {/* Block 5: Risk bands (v4) */}
      {data.universityRiskBands && data.universityRiskBands.length > 0 && (
        <div>
          <div className={`${styles.h3} ${styles.mb12}`}>
            대학별 리스크 밴드
          </div>
