...
3. **학년(year)과 학기(semester) 판단법 (가장 중요한 규칙)**:
   - 1학년=1, 2학년=2, 3학년=3 (숫자만)
   - 생기부 교과학습발달상황은 **학년 단위로 묶여** 있습니다. "[1학년]", "[2학년]", "[3학년]" 또는 "1학년 교과학습발달상황" 같은 헤더로 시작합니다.
   - 학년 판단 절차:
     1. 먼저 문서에서 학년 헤더("[1학년]", "[2학년]" 등)의 위치를 모두 파악하세요.
     2. 각 과목이 어느 학년 헤더와 어느 학년 헤더 사이에 있는지 확인하세요.
     3. 해당 구간의 학년을 그 과목의 year로 지정하세요.
   - **⚠️ 페이지가 넘어간다고 학년이 바뀌지 않습니다.** 한 학년의 성적이 여러 페이지에 걸쳐 있을 수 있습니다. 반드시 학년 헤더를 기준으로 판단하세요.
   - **⚠️ "학기"와 "학년"을 혼동하지 마세요.** "1학년 2학기"는 year=1, semester=2입니다. "2학기"를 보고 year=2로 넣으면 안 됩니다. 학년 헤더가 "[1학년]"이면 그 안의 1학기와 2학기 과목 모두 year=1입니다.
   - **⚠️ 각 학년은 1학기와 2학기 과목이 모두 있어야 합니다.** 1학년에 1학기만 있고 2학기가 없다면 학기를 학년으로 잘못 분류한 것입니다 (3학년 재학생만 예외). 출력 전 반드시 확인하세요.
   - 학기 판단: 각 학년 안에서 과목은 1학기 → 2학기 순서로 나열됩니다. 성적표에 "1학기", "2학기" 구분이 명시되어 있으면 그대로 따르세요.
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── 학년/학기 검증 + 교정 ──
    // 학년/학기 이상 징후 감지 (로그만, 교정은 하지 않음 — Gemini 프롬프트에서 처리)
    {
      const sections = [
        "generalSubjects",
        "careerSubjects",
