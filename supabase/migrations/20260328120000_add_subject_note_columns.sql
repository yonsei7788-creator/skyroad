-- 공동교육과정 비고 필드 지원을 위한 note 컬럼 추가
ALTER TABLE public.record_general_subjects ADD COLUMN IF NOT EXISTS note text;
ALTER TABLE public.record_career_subjects ADD COLUMN IF NOT EXISTS note text;

-- 잘못된 시그니처의 함수가 존재할 수 있으므로 제거
DROP FUNCTION IF EXISTS public.upsert_record(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb);

-- 원본 시그니처 유지 + note INSERT 추가
-- 기존 record가 orders에서 참조될 수 있으므로 삭제 대신 하위 데이터만 교체
...
BEGIN
  IF p_existing_record_id IS NOT NULL THEN
    -- 기존 레코드가 orders에서 참조될 수 있으므로 삭제 대신 하위 데이터만 삭제 후 재사용
