    ...
    case "majorExploration": {
      ...
      if (Array.isArray(explorationSuggestions)) {
        ...
        const GROUP_DEFAULT_DEPARTMENTS: Record<string, string[]> = {
          // 의예과/의학과는 등급 미달 시 차단되므로, 같은 임상·의료 지향이되
          // 선발 기준이 낮은 의생명 계열 내 학과를 추가로 확보해 인접 계열
          // (약학/바이오 등) 없이도 자체적으로 3개를 채울 수 있게 한다.
          의생명: [
            "의생명과학과",
            "의예과",
            "의학과",
          ...
          교육: ["교육학과", "교육공학과", "평생교육학과"],
          예체능교육: ["체육교육과", "음악교육과", "미술교육과"],
          예체능: ["연극영화학과", "디자인학과", "체육학과", "음악학과"],
          미디어: ["미디어커뮤니케이션학과", "신문방송학과", "광고홍보학과"],
        };

        // 인접 그룹 (1순위 그룹 자체 default만으로 3개를 못 채울 때의 최후 보충용)
        // ⚠️ 서로 다른 지향(예: 생명과학=연구, 약학=제약, 의생명=임상, 바이오=산업응용)을
        // 섞어서 "대분류로 엮어버리는" 문제를 막기 위해, 실제로 진로 지향이
        // 겹치는 관계로 근거가 명확한 경우에만 좁게 등록한다.
        // 의생명만 예외적으로 남기는 이유: 의예과/의학과가 등급 미달로 차단되는
        // 경우가 흔한데, 같은 임상·의료(간호보건) 지향 내에서 대체 학과를 찾는 것이
        // 전혀 무관한 계열(약학=제약, 바이오=산업)로 채우는 것보다 학생의 실제
        // 진로 지향에 부합한다. 위 GROUP_DEFAULT_DEPARTMENTS 확장으로 이 분기가
        // 실제로 필요한 경우는 이제 드물다.
