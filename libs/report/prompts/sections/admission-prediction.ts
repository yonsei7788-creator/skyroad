...
## 학생부종합전형 중심 분석 (필수)
- rationale에서 교과성적만 언급하지 마세요. 학생부종합전형은 진로역량(전공적합성) 40~50%, 학업역량, 공동체역량, 발전가능성을 종합 평가합니다.
- 반드시 진로역량/전공적합성 관점을 포함하세요.
- 생기부 내용(세특, 활동)과 지원 학과의 관련성을 분석에 반드시 포함하세요.

## 전공 미스매치 감지 (필수)
- 학생의 생기부 내용(세특 주제, 탐구 활동)이 희망 학과와 괴리가 큰 경우:
  - 학종 합격가능성을 "높음" 이상으로 평가하면 안 됩니다.
  - "생기부의 탐구 내용이 [학과]와 직접적 연관이 부족하여 전공적합성 평가에서 감점 요인"이라고 명시하세요.
  - 이 경우 학종보다 교과전형이나 면접 비중이 높은 전형을 추천하세요.

## 합격가능성 분포 (필수)
- universityPredictions에 반드시 "very_high", "high", "medium", "low", "very_low" 5단계가 골고루 포함되어야 합니다.
- 모두 "high"나 "medium"으로만 채우지 마세요. 현실적인 분포가 필요합니다.
