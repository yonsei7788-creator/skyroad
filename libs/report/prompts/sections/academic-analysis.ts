...
3. **gradeChangeAnalysis** (필수): **합격선 도달을 위한 남은 학기 목표**
   ...
   - prediction: 학생 입력의 마지막 학기 정보를 보고, 아직 이수하지 않은 가장 가까운 학기에서 평균 N등급을 달성하면 최종 평균이 X등급이 되어 합격선 도달 가능 형태로 작성. 졸업생이거나 모든 학기 이수 완료 시에는 "현재 평균 X등급은 합격선 대비 ~한 위치" 형태로 작성. 특정 학년·학기 표현은 학생 프로필의 "현재 시점", "입력된 마지막 학기" 정보와 일치하는 시점만 사용합니다.
