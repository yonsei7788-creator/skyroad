...
- 각 phase의 tasks는 **최대 3개**이며, 각 task는 **100자 이내**로 작성합니다.
...
- 로드맵은 **면접 대비, 수능 준비, 지원 전략 수립** 중심으로 작성하세요.
- 예: "1단계: 생기부 분석을 통한 면접 예상 질문 정리", "2단계: 면접 시 효과적인 설명 관점 정리", "3단계: 원서 접수 및 최종 전략 확정"`
    : grade && grade <= 2
...
- 의·치·한·약·수는 경쟁이 매우 치열합니다. 로드맵에 핵심 교과 성적 관리와 세특 심화 탐구 일정을 반드시 포함하세요.
...
## 서술 관점: 실행 플래너
이 섹션은 **시기별 구체적 행동 계획**을 제시합니다. 무엇을 언제까지 해야 하는지를 명확히 서술하세요.
- "~월까지 ~를 완료하세요", "~시기에는 ~에 집중해야 한다" 등 실행 계획 어투를 사용하세요.
...
- ⚠️ 이 금지는 **goals, tasks, description, completionStrategy 등 모든 출력 필드**에 적용됩니다. goals에 "수학 성적 개선" 같은 항목을 넣으면 안 됩니다.
- ✅ 로드맵에서는 구체적인 실행 계획(세특 심화 탐구, 동아리 활동, 면접 준비 등)만 제시하세요. "수학 성적 개선" 대신 "수학 교과 세특에서 경제 모델링 탐구 심화"처럼 활동 중심으로 작성하세요.
- ✅ 이 섹션에서 할 것: **구체적 실행 항목과 시기**만. 약점을 언급해야 할 경우 "~보완을 위해"로 짧게 전제하고 즉시 실행 계획으로 넘어가세요.
- ⚠️ **표현 차별화**: 약점이나 성적을 언급할 경우, "실행 계획의 동기"로만 1문장 이내로 언급하고 즉시 구체적 활동으로 넘어가세요. 예: ❌ "탐구 깊이가 부족하므로 보완이 필요합니다" → ✅ "탐구 심화를 위해 2학년 1학기에 사회탐구 소논문을 작성합니다"
...
## 조언 작성 원칙
- ⛔ **tasks와 goals에서 추천하는 활동·탐구 주제는 약점 분석과 입시 전략에서 감지된 강점 계열과 관련되어야 합니다.** 강점 계열과 무관한 분야의 활동(예: 정치외교 강점인데 마케팅/광고 활동 제안)을 추천하면 안 됩니다.
- ⛔ **goals에 "성적 개선", "성적 향상", "등급 끌어올리기" 등 성적 목표를 넣지 마세요.** goals는 활동/탐구 중심으로만 작성합니다. 예: ❌ "수학 성적 개선" → ✅ "수학 교과 세특 탐구 심화"
