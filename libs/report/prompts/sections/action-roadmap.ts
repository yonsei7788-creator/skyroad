...
export const buildActionRoadmapPrompt = (
  ...
  // ─── 한국 대입 일정 가드 ───
  // 수시 원서접수: 9월 초 / 생기부 마감: 8월 31일
  // 고3은 그 해 8월이 마지막 생기부 마감. 9월 이후엔 생기부 수정/활동 추가 불가.
  // 고2는 다음 해 8월이 마감 → 현재 시점에서는 모두 마감 전이지만,
  //   "고3 9월 이후" 시점의 phase를 만들 때는 생기부 활동을 넣으면 안 됨.
  const isHigh3AfterGiboomEnd =
    grade === 3 && currentMonth >= 9 && !input.isGraduate;
  const isHigh3BeforeGiboomEnd =
    grade === 3 && currentMonth <= 8 && !input.isGraduate;
...
- 예: "1단계: 생기부 분석을 통한 면접 예상 질문 정리", "2단계: 면접 시 효과적인 설명 관점 정리", "3단계: 원서 접수 및 최종 전략 확정"`
    : isHigh3AfterGiboomEnd
      ? `\n## ⚠️⚠️⚠️ 생기부 마감 후 시점 (최우선 가드)
- 이 학생은 **고3 + ${currentMonth}월** 시점입니다. 한국 대입 수시 원서접수는 9월 초이고, **생기부는 ${currentYear}년 8월 31일자로 이미 마감**되었습니다.
- ⛔ "세특 보완", "동아리 활동", "탐구 주제 추가", "수업 중 발표", "교과 활동 추가" 등 **생기부에 기록될 활동 제안을 절대 하지 마세요.** 이미 작성 불가능한 항목입니다.
- ⛔ phases에 "${currentYear}년 ${currentMonth}월~12월에 ~를 하세요" 같은 생기부 관련 활동 제시 금지.
- ✅ phases는 **면접 대비, 수능 마무리, 정시 지원 전략** 중심으로만 구성하세요. 보통 2~3개 phase면 충분합니다.
- ✅ 예시: "면접 대비기(${currentYear}년 ${currentMonth}월~수능 전)", "수능 직전기(11월)", "정시 지원기(12월~${currentYear + 1}년 1월)"`
      : isHigh3BeforeGiboomEnd
        ? `\n## ⚠️⚠️⚠️ 생기부 마감 임박 (고3 가드)
- 이 학생은 **고3 + ${currentMonth}월** 시점입니다. 한국 대입은 **${currentYear}년 8월 31일에 생기부 마감 → 9월 초 수시 원서접수**입니다.
- ⛔ phases의 period에 "${currentYear}년 9월", "${currentYear}년 10월", "${currentYear}년 11월", "${currentYear}년 12월" 같은 시기에 **세특/탐구/동아리/교과 활동 등 생기부 관련 활동을 절대 넣지 마세요.** 9월 이후는 생기부 마감 후입니다.
- ✅ 생기부 활동(세특/탐구/동아리)은 반드시 **${currentYear}년 8월까지** 마무리하는 phase에만 배치하세요.
- ✅ 9월 이후 phases는 **면접 대비, 수능 마무리, 정시 지원 전략** 중심으로 작성하세요.
- ✅ 권장 구조: "현재(${currentMonth}월)~8월 생기부 마무리" + "9월~수능 면접/수능 대비" 2개 phase면 충분합니다.`
        : grade === 2
          ? `\n- 이 학생은 2학년입니다. 다음 해 8월 31일이 생기부 마감이고, 그 직후 9월 초 수시 원서접수입니다.
- "3학년 1학기", "3학년 ${currentYear + 1}년 8월" 등을 지칭하지 말고 "남은 학기", "앞으로의 기간" 유연한 표현 사용.
- ⛔ phases의 period에 "${currentYear + 1}년 9월" 이후 시점에 **세특/탐구/동아리 등 생기부 관련 활동을 절대 넣지 마세요** (그때는 이미 생기부 마감 후입니다). 9월 이후 시점이 phase에 포함된다면 면접/수능 대비 내용으로 채우세요.
- ❌ "3학년 1학기 내신을 1등급으로" → 아직 2학년이므로 부적절합니다.
- ✅ "남은 기간 성적을 끌어올려야 합니다", "다음 학년에서 ~를 보완"`
          : grade === 1
            ? `\n- 이 학생은 1학년입니다. 다다음 해 8월 31일이 생기부 마감입니다.
- ⛔ phases의 period에 "${currentYear + 2}년 9월" 이후 시점에 **세특/탐구/동아리 등 생기부 관련 활동을 절대 넣지 마세요.**`
            : "";

  // 현재 시점 + 학년 기준 phase 예시를 동적 생성
  // 핵심 원칙:
  //   - 고3 + 9월 이후: 생기부 마감 후 → 면접/수능/정시 대비만
  //   - 고3 + 8월 이전: 8월까지 생기부 마무리 + 9월~수능 면접/수능 대비
  //   - 고2 이하: 기존 학기 흐름 유지 (다음 해 8월 31일이 마감 → 그 이후 phase에는 생기부 활동 X)
  const examplePhases = (() => {
    const m = currentMonth;
    const y = currentYear;

    // 고3 + 8월 이후: 생기부 마감 후 — 면접/수능/정시 phases만
    if (isHigh3AfterGiboomEnd) {
      return [
        {
          phase: "면접 대비기",
          period: `${y}년 ${m}월~수능 전`,
          goals: ["지원 대학별 기출/예상 질문 정비"],
          tasks: [
            "학생부 기반 면접 예상 질문 100개 정리",
            "지원 대학 면접 평가 항목 분석 및 모의 면접 진행",
          ],
        },
        {
          phase: "수능 직전기",
          period: `${y}년 11월`,
          goals: ["수능 영역별 마무리"],
          tasks: ["취약 영역 기출 분석", "실전 모의고사 시간 관리 훈련"],
        },
        {
          phase: "정시 지원기",
          period: `${y}년 12월~${y + 1}년 1월`,
          goals: ["정시 지원 전략 확정"],
          tasks: ["수능 점수 기반 정시 가/나/다군 조합 시뮬레이션"],
        },
      ];
    }

    // 고3 + 8월 이전: 현재~8월 생기부 마무리 + 9월~수능 면접/수능
    if (isHigh3BeforeGiboomEnd) {
      return [
        {
          phase: "생기부 마무리기",
          period: `${y}년 ${m}월~8월`,
          goals: ["3학년 1학기 세특 핵심 탐구 완성"],
          tasks: [
            "수업 중 핵심 탐구 발표 마무리",
            "8월 말 생기부 마감 전 동아리 결과물 정리",
          ],
        },
        {
          phase: "면접·수능 대비기",
          period: `${y}년 9월~수능 전`,
          goals: ["면접 준비 + 수능 영역별 마무리"],
          tasks: [
            "지원 대학별 면접 기출 분석 및 모의 면접",
            "수능 취약 영역 집중 학습",
          ],
        },
      ];
...
4. **생기부 마감 가드**: 고3은 ${currentYear}년 8월 31일이 생기부 마감, 고2는 ${currentYear + 1}년 8월 31일이 마감입니다. 마감 후 시점(고3은 ${currentYear}년 9월 이후, 고2는 ${currentYear + 1}년 9월 이후)의 phase에 세특/탐구/동아리/교과 활동 등 생기부 관련 과제가 들어 있으면 즉시 면접/수능/정시 대비 항목으로 교체하세요.
${gradeContext}
