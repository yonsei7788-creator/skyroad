...
- ❌ 창체 활동 평가 → activityAnalysis에서 다룸
- ✅ 이 섹션에서 할 것: **담임 기록에 드러난 인성·태도·리더십 특성**만 추출
...
각 학년에 대해:
- year: 학년
- summary: 핵심 서술 요약 (2~3줄). **사정관 평가 문장은 반드시 \`[[insight]]...[[/insight]]\`로 감쌉니다** (시스템 프롬프트의 사정관 평가 마커 규칙 참조).
...
- 짧은 키워드이므로 마커는 사용하지 마세요.

### 종합 평가 (overallComment)
- 행동특성이 전달하는 학생 이미지를 종합합니다 (3~5줄).
- **사정관 평가 문장은 반드시 \`[[insight]]...[[/insight]]\`로 감쌉니다.** 단순 종합 서술이 아닌, "사정관 입장에서 어떻게 평가할지 / 면접에서 어떤 질문이 나올지 / 변별력 / 감점 요인"을 담은 문장.

### 입시 활용 포인트 (admissionRelevance)
- 면접에서 행동특성 내용을 어떻게 활용할 수 있는지 조언합니다 (3~5줄).
- 특히 인성 면접 질문과 연결 가능한 에피소드를 식별합니다.
- **이 필드 자체가 사정관 평가 관점이지만, 그 안에서도 핵심 평가 문장은 \`[[insight]]...[[/insight]]\`로 감싸 강조합니다.**
