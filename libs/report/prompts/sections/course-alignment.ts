...
- recommendation은 위 "학년·시점별 분석 방향" 블록에서 학생 상태에 맞게 안내한 트랙(고1·고2: 잔여 학기 이수 / 3학년 재학생: 트랙 A·B / 졸업생: 면접 대응)을 그대로 따라 작성합니다.
- 미이수 과목이 여러 개일 때는 각각의 과목에 대해 어떤 트랙으로 다룰지 따로 판단해 한 단락으로 통합 서술합니다.

## 미이수 과목 대응 시나리오 요약
- **고1·고2 재학생**: 두 시나리오 모두 작성합니다.
  - A. 향후 수강할 경우: 해당 과목을 앞으로 이수한다면 진로역량·입시 경쟁력에 어떤 긍정적 변화가 있는지 서술합니다.
  - B. 미이수가 유지될 경우: 해당 과목 없이 지원할 때의 불이익과 이를 보완할 수 있는 대안 전략을 제시합니다.
- **3학년 재학생**: 위 "학년·시점별 분석 방향"의 트랙 A·B를 그대로 적용합니다. 3학년 동안 새로 권장과목을 선택하는 시나리오는 다루지 않습니다.
