...
${
  ...
  "recommendation": "이미 이수한 경제 과목 세특에서 사회과학 핵심 주제 탐구를 어떻게 풀어냈는지 면접에서 설명할 수 있도록 정리하세요. 세계사는 이수하지 않았으므로 세계사적 관점을 다른 사회 교과 세특·동아리 탐구로 우회 보완한 사례를 정리합니다."
}`
    : `{
  "sectionId": "courseAlignment",
  "title": "과목 적합도",
  ...
  "recommendation": "수강 예정 과목에 포함된 경제는 그 과목 세특에서 사회과학 핵심 주제 탐구를 어떻게 풀지 설계하면 효과를 극대화할 수 있습니다. 세계사는 수강 예정에 없으므로 세계사적 관점을 다른 사회 교과 세특·동아리 탐구로 우회 보완해야 합니다."
}`
}
...
${
  input.isGraduate === true || input.studentGrade === 3
    ? `- takenCourses(이수 완료) 과목은 "이미 이수한 OO 과목 세특에서 면접 시 어떤 탐구·발표 내용을 풀어 설명할지" 형태로 서술합니다. "수강 예정", "이수 예정", "잔여 학기에 이수하면" 같은 표현 금지.
- missingCourses는 시간표가 확정된 시점이므로 우회 보완(다른 교과 세특·동아리·자율탐구·독서 등) + 면접 대응(미이수 사유 설명) 방향으로 서술합니다.
- 미이수 과목이 여러 개일 때는 한 단락으로 통합 서술합니다.`
    : `- "이수 예정"으로 분류된 권장과목은 **수강이 확정된 과목**으로 다루세요. "OO를 이수하는 것이 필수적입니다", "OO를 이수해야 합니다" 같은 표현 대신 "수강 예정인 OO 세특에서 어떤 탐구·발표 방향을 잡으면 좋을지" 형태로 서술합니다.
