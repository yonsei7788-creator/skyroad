...
- 이 학생은 9등급제 적용 학생입니다. 과목명은 반드시 2015 개정 교육과정 기준으로 작성하세요.`;
  ...
  const medicalCourseContext = input.isMedical
    ? `
## ⚠️ 메디컬 계열 과목 선택 특화 가이드 (반드시 적용)

이 학생은 의·치·한·약·수 계열(메디컬) 지원 학생입니다.

### 메디컬 핵심 과목 이수 기준
- **수학**: 미적분(2015) / 미적분Ⅰ·Ⅱ(2022)는 메디컬에서 사실상 필수입니다. 확률과 통계만 이수한 경우 학종에서 "전공 관련 교과 이수 노력 부족"으로 평가될 수 있습니다.
- **과학**: 생명과학Ⅱ·화학Ⅱ(2015) / 세포와 물질대사·물질과 에너지(2022) 등 심화 과학 이수 여부가 메디컬 학종의 핵심 평가 요소입니다.
- **영어**: 메디컬에서 영어 1등급은 사실상 필수 조건입니다. 과목 이수 분석에서 영어 성취도도 함께 언급하세요.

### 사탐 메디컬 현상과 과목 선택 전략 (2026학년도 기준)
최근 메디컬 계열에서 사회탐구/확률과 통계 선택자가 증가하고 있습니다:
- 의예과: 사탐 응시자 9.3%
...
#### 과탐 선택의 장점
- 대부분의 메디컬 대학이 과탐 응시자에게 가산점 부여
- 미적분·기하 + 과탐 조합이 전통적으로 메디컬 지원의 정석
...
1. **고1~2 학생 (필수)**: 다음 문장을 recommendation에 반드시 포함:
   "정시 과목 선택 전략도 중요합니다. 2026학년도 기준 메디컬 사탐 응시자 비율은 의예과 9.3%, 한의예과 74.8%, 수의예과 40.5%, 약학 23.8%로, 사탐 메디컬 현상이 확대되고 있습니다. 단, 확률과 통계는 상위권 점수 집중도가 높고 표준점수 최고점이 미적분·기하 대비 낮으므로, 단순 난이도가 아닌 표준점수 구조 + 응시 집단 분포 + 목표 대학 반영 방식을 종합 고려하여 선택해야 합니다."

2. **과탐 미이수 + 메디컬 지원**: "학종에서 전공 관련 교과 이수 노력 평가에서 불리할 수 있습니다. 과탐 이수를 적극 권장합니다." 반드시 추가
...
- **targetMajor는 아래 역량 추출 결과의 detectedMajorGroup을 그대로 사용하세요.**
...
### 목표 계열 (targetMajor)
- 역량 추출 결과의 detectedMajorGroup 값을 그대로 사용합니다.
