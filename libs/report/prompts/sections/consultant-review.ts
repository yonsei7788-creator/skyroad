...
`
  ...
  const analysisProcedure = isGyogwaOnly
    ? `## 분석 절차
1. **최종 등급 분석**: 최종 평균 등급과 주요 과목별 등급만으로 교과전형 합격선 대비 위치를 해석합니다. 학기별 등급을 나열하거나 상승/하락 추세를 언급하지 마세요. "최종 평균 N등급은 ~합격선에 미달/도달합니다" 형태로만 서술하세요.
2. **교과 이수 노력 평가**: 전공 관련 핵심 과목의 이수 여부, 성취도, 과목 선택의 전략성을 평가합니다.
3. **전형 전략 수립**: 최종 등급과 교과전형 합격선을 비교하여 지원 가능 대학 수준을 판단합니다. 교과전형은 최종 등급만 보므로, 성적의 상승/하락 추세를 전략에 포함하지 마세요.
   - 교과전형 외에 정시 병행 전략도 함께 제시합니다.
