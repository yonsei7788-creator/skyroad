...
- gradeAnalysis: **500자 이내**. 수강자수/표준편차/원점수 기반 정밀 분석, 과목 간 등급 편차의 사정관 해석, 성적 추이의 전략적 의미. ⚠️ 고교 유형 보정(환산 등급) 수치나 보정 사실을 텍스트에 직접 언급하지 마세요.
...
5. **전형 전략 수립**: 성적 구조 + 비교과 역량 + 학교 유형을 종합하여 최적 전형 전략을 제시합니다. ⚠️ 교과전형 분석 시에는 원래 등급(original) 기준으로 판단하세요. 교과전형은 고교 유형 구분 없이 내신을 그대로 산출합니다. 학종 분석 시에만 환산 등급을 내부 참고하되, 텍스트에 환산 등급 수치나 보정 사실을 언급하지 마세요.
...
\`\`\`json
{
  ...
  "evaluationGuide": {
    ...
    "analysisMethodology": "진로역량 분석(세특 개수, 활동, 탐구 주제, 교과 선택 패턴)과 학업 흐름 분석(성적 변화, 학업 태도, 교과 선택, 세특 내용)이 이 학생에게 어떻게 적용되었는지..."
  }
}
\`\`\`
