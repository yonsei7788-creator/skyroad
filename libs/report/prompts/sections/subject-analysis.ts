...
## ⭐ 사정관 평가 마커 (필수)
subjects[].evaluationComment, subjects[].detailedEvaluation, subjects[].improvementDirection, subjects[].sentenceAnalysis[].evaluation 등 **모든 자유서술 평가 필드**에서 사정관 평가 문장은 반드시 \`[[insight]]...[[/insight]]\`로 감싸세요.
...
- 예시: "복지 정책의 효과성을 KOSIS 데이터로 검증한 탐구가 기록되어 있습니다. [[insight]]데이터 기반 사고가 드러나 학종 서류평가에서 탐구력 항목 가점이 예상되지만, 정책 분석의 한계점 인식이 빠져 면접에서 '데이터의 신뢰도를 어떻게 검증했는가' 질문이 나올 가능성이 높습니다.[[/insight]]"
- activitySummary(활동 요약) 필드는 평가가 아닌 사실 요약이므로 마커를 사용하지 마세요.
...
## 규칙
1. ⛔ **subjectName 강제 규칙 (최우선 — 위반 시 품질 실패)**
   - subjects[].subjectName은 반드시 "교과 세특 원문 데이터"의 \`[N학년 과목명]\` 라벨에 등장한 과목명을 **그대로 복사**하세요.
   - 본문 내용을 보고 다른 과목명으로 추측·변환·재분류하지 마세요.
   - ✅ 입력에 \`[1학년 과학탐구실험]\`이 있으면 subjectName: "과학탐구실험"
   - ❌ 입력에 "물리학Ⅰ"이 라벨로 없는데 본문 내용(예: 아치 구조의 힘 분산)을 보고 subjectName을 "물리학1"·"물리학Ⅰ"로 적으면 환각 — 즉시 품질 실패
   - 같은 학년-과목명 라벨이 여러 학기에 걸쳐 있으면 학년만 갱신해 1개로 통합하세요.
2. **교과 세특만** 분석합니다.
3. 평가는 반드시 원문 내용에 근거합니다.
4. 역량 태깅은 가장 두드러지는 역량 1~3개를 선택합니다.
5. 학년-과목명 형식으로 과목을 식별합니다 (입력 \`[N학년 과목명]\` 라벨을 그대로 사용).
6. **"과목다운 세특" 관점을 반드시 적용합니다**: 해당 과목의 본질적 특성에 맞는 세특인지 평가합니다.
   - 수학 세특에는 수학적 사고/증명/모델링이, 물리학 세특에는 실험/현상 분석이 드러나야 합니다.
   - 과목 특성과 무관한 단순 소재 활용(예: 물리학 세특인데 자동차 역사 조사)은 감점 요인입니다.
7. **활동 밀도를 분석합니다**: 1500바이트(약 500자) 내 3개 이상 활동 나열은 깊이 부족으로 판단합니다.
   - 활동 수 대비 분량을 확인하고, 밀도가 낮은(=활동 수는 적지만 깊이 있는) 세특을 높이 평가합니다.
8. **학업 역량 판단 기준**: 세특에서 다음 패턴이 나타나면 학업역량(탐구력/학업태도)에서 높은 평가를 받는 학생입니다:
...
9. **평가자 관점 필수 (최우선)**: 시스템 프롬프트의 '서술 vs 평가' 기준을 적용하세요. 원문 요약·반복 금지, 반드시 평가 판단 + 세특 원문의 구체적 근거를 함께 서술하세요.
...
10. **개선 예시 문체 규칙**: improvementExample 등 세특 서술 예시는 반드시 "~함.", "~임.", "~됨." 등 세특 문체로 작성합니다. 존댓말("~합니다", "~입니다")은 세특 예시에서 절대 사용하지 마세요.
11. **요약-인용 중복 제거**: 과목명 하단의 요약 서술(activitySummary, 2줄 분량)과 핵심 인용(keyQuotes) 내용이 중복되는 경우, 요약 서술을 생략하고 핵심 인용만 남기세요.
12. **입학사정관 해석 관점**: evaluationComment 또는 detailedEvaluation에서 사정관이 이 세특을 읽었을 때 어떻게 평가할지를 자연스럽게 포함하세요.
...
13. **형식적 표현 패턴 탐지 (필수)**: 시스템 프롬프트의 '형식적 표현 패턴' 기준을 적용하여, 세특에서 해당 표현이 발견되면 evaluationComment에서 반드시 단점으로 지적하세요. 최소 절반 이상의 과목에서 표현상 한계를 지적해야 합니다. (단, **1학년 세특**에서는 탐색기 특성상 감점 수준을 낮추세요.)
14. **탐구 4단계 체크리스트 (필수)**: 시스템 프롬프트의 '탐구 구조 4단계' 기준을 적용하여 각 세특의 탐구 수준(조사/탐구/심화 탐구)을 판단하고 evaluationComment에 반영하세요.
15. **학년 간 심화 흐름 판단**: 같은 교과군 내에서 1학년 → 2학년으로 주제가 심화되었는지 확인합니다.
