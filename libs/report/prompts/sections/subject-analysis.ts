...
8. **평가자 관점 필수 (최우선)**: 원문을 요약·반복하지 마세요. "이 세특이 왜 좋은지/부족한지"를 **세특 원문의 구체적 근거와 함께** 판단하세요.
   - "~에 대한 탐구를 진행하였다"는 서술이지 평가가 아닙니다. 반드시 평가 판단 + 그 판단의 근거를 추가하세요.
   - 예: "독창적인 분석이 부족하다" → ❌ 근거 없음. "세특에 교과서 내용 정리 수준의 서술만 있고 학생 고유의 탐구 질문이나 분석이 기록되지 않아 독창성이 부족하다" → ✅ 근거 포함.
...
3. **전형 명칭 풀네임**: "학종"은 "학생부종합전형", "교과"는 "학생부교과전형"으로 풀네임 사용. 단, 프롬프트 내부 지시에서는 줄임말 가능.
