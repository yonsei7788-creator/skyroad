...
export const buildSubjectAnalysisPrompt = (
  ...
  const medicalSubjectContext = input.isMedical
    ? `## ⚠️ 메디컬 계열 세특 분석 가이드 (반드시 적용)

이 학생은 의·치·한·약·수 계열(메디컬) 지원 학생입니다.

### ⛔ 세특 평가의 핵심 원칙 (최우선)
- 세특 평가는 **"이 활동이 어떤 역량을 보여주는가?"**를 기준으로 합니다.
- ❌ "메디컬 계열 지원에 매우 적합한 탐구" — 희망학과 기준 평가 금지
- ❌ "메디컬 지원에 필요한 탐구 깊이" — 희망학과 잣대로 재기 금지
- ✅ "질병의 병리적 기전을 심층 분석하고 기술의 한계를 비판적으로 인식한 점은 탐구력이 매우 우수합니다" — 역량 중심 서술
- ✅ "실험 기반의 과학적 방법론이 체계적으로 적용되어 학업역량에서 긍정적입니다" — 역량 중심 서술
- evaluationComment에 **"메디컬 계열 지원에 적합"이라는 표현을 사용하지 마세요.** 대신 탐구력, 분석력, 자기주도성 등 역량 용어로 평가하세요.

### 메디컬 학생의 세특 채점 시 추가 기준
