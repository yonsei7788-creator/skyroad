...
export const matchMajorEvaluationCriteria = (
  ...
  // ⚠️ 자연과학/공학 매칭보다 반드시 먼저: "음악교육"이 "교육"으로 오매칭되거나
  //   "영화"가 "화학"의 부분일치로 오매칭되는 것을 방지
  if (/체육교육|음악교육|미술교육|무용교육/.test(lower)) {
    return findCriteria("예체능교육");
  }

  // 예체능 계열 (순수 예체능) — 자연과학/공학보다 반드시 먼저 체크
  // ⚠️ 정규식 부분일치 함정: "연극영화학과"는 "화학과"의 substring("화학과")을 포함하여
  //   자연과학 정규식에 잘못 매칭됨. 따라서 예체능 키워드를 가장 먼저 차단해야 함.
  if (
    /예술|미술|음악|체육|무용|디자인|영화|연극|영상|연출|시나리오|공연|실용음악|작곡|회화|조소|방송영상|애니메이션|만화|패션|뷰티|사진|게임|모델|성악|국악|기악|연기|뮤지컬|연희|판소리|한국화|서양화|조형|공예|스포츠/.test(
      lower
    )
  ) {
  ...
  // 자연과학 계열
  // ⚠️ "화학과" 정규식이 "영화학과"의 substring과 매칭되지 않도록 negative lookbehind 사용
  //   ((?<![영변순진]) — "영화학과", "변화학", "순화학", "진화학" 등 오매칭 차단)
  if (
    /수학과|물리[학과]|(?<![영변순진])화학과|생[명물]|천문|지구과학|통계학/.test(
      lower
    )
  ) {
  ...
  // 인문 계열
  if (
    /국[어문]|영[어문]|불[어문]|독[어문]|일[어문]|중[어문]|사학|역사|철학|문학|어문|인문|문헌정보|문예창작|창작|언어학|종교|신학/.test(
      lower
    )
  ) {
    return findCriteria("인문");
